 ...
 ******************************************************************/
  ...
  const generateAndDownloadLOI = async () => {
    ...
    // Profile validation is now handled early when profile loads
    ...
    try {
      ...
      // Handle Purchase & Sale Agreement generation separately
      if (loiType === 'purchase_sale') {
        const userProfileForPSA = {
          name: `${userProfile?.first_name || ''} ${userProfile?.last_name || ''}`.trim(),
          phone: userProfile?.phone_number || '',
          email: currentUser?.email || '',
          address: userProfile?.street_address || '',
          cityzip: `${userProfile?.city || ''}, ${userProfile?.state || ''} ${userProfile?.zipcode || ''}`.trim(),
          business_name: userProfile?.business_name,
          business_title: userProfile?.job_title,
          logoBase64: userProfile?.logo_base64 || undefined
        };
        
        // Create a simple activity tracking function
        const incrementActivity = async (activityType: string) => {
          // TODO: Implement activity tracking for P&S agreements
          console.log(`Activity tracked: ${activityType}`);
        };
        
        await generatePurchaseAndSaleAgreement(
          data,
          userProfileForPSA,
          formatCurrencyJS,
          numberToWords,
        ...
        // Show success toast
        if (typeof window !== 'undefined' && (window as any).toast) {
          (window as any).toast.success('Purchase & Sale Agreement generated successfully!');
        }
        ...
        return; // Exit early for P&S Agreement
      }
  ...
  // Show loading while profile is being validated
  if (isLoadingProfile) {
    return (
      ...
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-4xl border border-gray-200">
        ...
        <section className="mb-8">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            {/* Purchase & Sale Agreement Card */}
            <label className={`relative flex flex-col p-6 rounded-lg border-2 cursor-pointer transition-all h-full ${loiType === 'purchase_sale' ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'
              }`}>
              <input
                type="radio"
                className="sr-only"
                name="loiType"
                value="purchase_sale"
                checked={loiType === 'purchase_sale'}
                onChange={() => setLoiType('purchase_sale')}
              />
              <div className="flex items-center justify-between mb-3">
                <span className="text-lg font-semibold text-gray-800">Purchase & Sale</span>
                <div className={`w-5 h-5 rounded-full border-2 ${loiType === 'purchase_sale' ? 'border-blue-500 bg-blue-500' : 'border-gray-400'
                  }`}>
                  {loiType === 'purchase_sale' && (
                    <svg className="w-3 h-3 text-white mx-auto mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                    </svg>
                  )}
              ...
              <p className="text-sm text-gray-600 leading-relaxed">Comprehensive purchase agreement with detailed terms, contingencies, and legal protections. Perfect for finalizing negotiated deals.</p>
            </label>
          </div>
        </section>
