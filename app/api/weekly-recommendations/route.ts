...
import { generateWeeklyRecommendations, BuyBoxMarket } from '@/lib/v2/weeklyRecommendations';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const cronUserId = requestBody.userId;
    ...
    if (automatedUserId || cronUserId) {
      ...
      user = {
        id: userId,
        email: `user-${userId}@automated.trigger`
      };
    ...
    // Fetch available properties using cached approach
    const propertySearchRequests = convertedMarkets.map(async (market) => {
      ...
      const sampleSizes = { 1: 15, 2: 12, 3: 10, 4: 8 };
      const maxSample = sampleSizes[marketTier as keyof typeof sampleSizes] || 10;
      
      // Random sampling for diversity
      const shuffled = availableIds.sort(() => Math.random() - 0.5);
      const sampleIds = shuffled.slice(0, Math.min(maxSample, 50));
    ...
    // Generate recommendations for each market (3 per market)
    ...
    const allRecommendedProperties = validRecommendations.flatMap(rec => 
      rec.recommendations.map(scored => ({
        ...
        marketKey: rec.market.marketKey,
    ...
    try {
      await Promise.all(allRecommendedProperties.map(async ({ property, marketName, marketKey, fitScore, diversityScore, totalScore, reasons }) => {
        // Insert to saved_properties table (only new ones)
        ...
        // Upsert to user_favorites as pending recommendation
