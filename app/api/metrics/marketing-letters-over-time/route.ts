...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get marketing letter activity data for the specified time range
    const { data: activityData, error } = await supabase
      .from('user_activity_counts')
      .select('activity_date, count')
      .eq('user_id', userId)
      .eq('activity_type', 'marketing_letters_created')
      .gte('activity_date', startDate.toISOString().split('T')[0])
      .order('activity_date', { ascending: true });

    if (error) {
      console.error('Error fetching marketing letter activity data:', error);
      return NextResponse.json({ error: 'Failed to fetch marketing letter data' }, { status: 500 });
    }
    ...
    console.error('Error in marketing letters over time API:', error);
