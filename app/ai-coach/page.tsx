...
// Legacy attachment system removed - now using PDFAttachmentAdapter
...
const AICoachContent = ({ threads, loadingThreads, loadThread, createNewThread, deleteThread, runtime }: AICoachContentProps) => {
  ...
  return (
    <div className="flex flex-col bg-white" style={{ height: 'calc(100vh - 4rem)' }}>
      {/* Header Navigation - Hidden until next release */}
      {/* 
      ...
      */}
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar with Thread List - Hidden until next release */}
        {/* 
...
// Component that creates runtime with PDFAttachmentAdapter
const AICoachWithRuntime = () => {
  const { user, isLoading } = useAuth();
  const router = useRouter();
  const { runtime, threads, loadingThreads, loadThread, createNewThread, deleteThread } = usePersistedChatRuntime();
