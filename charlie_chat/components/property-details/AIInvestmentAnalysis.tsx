              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                ...
                <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                  ...
                  <div className="space-y-2 text-sm text-green-700">
                    ...
                    {investmentAnalysis.financialStrength.marketRent && (
                      <div className="flex justify-between">
                        <span>Market Rent:</span>
                        <span className="font-semibold">{investmentAnalysis.financialStrength.marketRent}</span>
                      </div>
                    )}
                  </div>
                  {investmentAnalysis.financialStrength.notes.length > 0 && (
                    <div className="mt-3 pt-2 border-t border-green-200">
                      {investmentAnalysis.financialStrength.notes.map((note: string, i: number) => (
                        <div key={i} className="text-xs text-green-600">• {note}</div>
                      ))}
                    </div>
                  )}
                </div>
                  ...
                  {investmentAnalysis.propertyFeatures.notes.length > 0 && (
                    <div className="mt-3 pt-2 border-t border-blue-200">
                      {investmentAnalysis.propertyFeatures.notes.map((note: string, i: number) => (
                        <div key={i} className="text-xs text-blue-600">• {note}</div>
                      ))}
                    </div>
                  )}
                </div>
                  ...
                  {investmentAnalysis.riskFactors.notes.length > 0 && (
                    <div className="mt-3 pt-2 border-t border-yellow-200">
                      {investmentAnalysis.riskFactors.notes.map((note: string, i: number) => (
                        <div key={i} className="text-xs text-yellow-600">• {note}</div>
                      ))}
                    </div>
                  )}
                </div>
