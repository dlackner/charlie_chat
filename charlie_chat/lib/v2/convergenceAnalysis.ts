...
const calculateMarketConvergence = (decisions: any[]): number => {
    ...
    console.log(`V2 Convergence analysis: ${confidenceScores.length} measures, avg=${finalConfidence.toFixed(3)} (${decisions.length} decisions)`);
...
const getMarketName = async (supabase: SupabaseClient, userId: string, marketKey: string): Promise<string | null> => {
    try {
        ...
        console.error('V2 Error getting market name:', error);
        return null;
    }
};
...
export const updateMarketConvergence = async (userId: string, supabase: SupabaseClient): Promise<{ [marketKey: string]: { phase: 'discovery' | 'learning' | 'mastery' | 'production'; progress: number } }> => {
    try {
        console.log('🔄 V2 Updating market convergence after weekly recommendations...');
        ...
        if (error) {
            console.error('V2 Error fetching user decisions for convergence:', error);
            return {};
        }

        if (!decisions || decisions.length === 0) {
            console.log('V2 No decisions found for convergence analysis');
            return {};
        }
        ...
        Object.entries(marketDecisions).forEach(([marketKey, marketDecisionList]) => {
            ...
            if (convergenceScore >= 0.8 && marketDecisionList.length >= 10) {
                ...
                if (currentState?.phase === 'mastery' && currentState?.masteryDate) {
                    ...
                    if (masteryDate <= fourWeeksAgo) {
                        ...
                        console.log(`🎯 V2 Market ${marketKey}: PROMOTED to Production after 4+ weeks of mastery!`);
                        console.log(`📢 V2 User will be notified about Production transition for ${marketKey}`);
                    } else {
                        // Still in mastery, waiting for 4-week period
                        phase = 'mastery';
                        progress = 3;
                        const daysInMastery = Math.floor((Date.now() - masteryDate.getTime()) / (1000 * 60 * 60 * 24));
                        console.log(`📊 V2 Market ${marketKey}: mastery (${daysInMastery}/28 days, score: ${convergenceScore.toFixed(3)})`);
                    }
                } else {
                    ...
                    console.log(`🎯 V2 Market ${marketKey}: ACHIEVED mastery! Starting 4-week stability period.`);
                }
                ...
                if (currentState?.phase === 'mastery') {
                    ...
                    console.log(`📊 V2 Market ${marketKey}: dropped from mastery to learning`);
                }
            } else {
                ...
                if (currentState?.phase === 'mastery') {
                    updates.mastery_achieved_date = null;
                    console.log(`📊 V2 Market ${marketKey}: dropped from mastery to discovery`);
                }
            }
            ...
            console.log(`📊 V2 Market ${marketKey}: ${phase} (score: ${convergenceScore.toFixed(3)}, decisions: ${marketDecisionList.length})`);
        });
        ...
        for (const { marketKey, updates } of marketUpdates) {
            ...
            if (updateError) {
                console.error(`V2 Error updating market ${marketKey}:`, updateError);
            }
        }
        ...
        console.error('V2 Error calculating market convergence:', error);
        return {};
    }
};