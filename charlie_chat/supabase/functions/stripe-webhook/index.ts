...
async function syncCustomerToProfile(customer, supabase) {
  ...
  if (Object.keys(profileUpdates).length > 0) {
    ...
    const { error } = await supabase.from('profiles').update(profileUpdates).eq('stripe_customer_id', customer.id);
    if (error) {
...
async function handleSubscription(session, profile, supabase, stripe) {
  ...
  const priceToClassMap = {
    ...
    [Deno.env.get('NEXT_PUBLIC_CHARLIE_CHAT_PLUS_MONTHLY_PRICE') ?? '']: "charlie_chat_plus",
    [Deno.env.get('NEXT_PUBLIC_CHARLIE_CHAT_PLUS_ANNUAL_PRICE') ?? '']: "charlie_chat_plus",
