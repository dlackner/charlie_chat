 ...
 ******************************************************************/
  ...
  const generateAndDownloadLOI = async () => {
    ...
    if (!userProfile || !currentUser?.email) {
      if (profileError) {
        alert(`Cannot generate LOI: ${profileError}. Please complete your profile first.`);
        return;
      }
      alert("Profile data is still loading. Please try again in a moment.");
      return;
    }
    ...
    const data = {
      ...
      yourCityZip: `${userProfile.city}, ${userProfile.state} ${userProfile.zipcode}`,
  ...
  if (isLoadingAuth || isLoadingAccess || !isLoggedIn) {
    return (
