    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [metricsResponse, activityResponse, marketsResponse] = await Promise.all([
          fetch(`/api/v2/dashboard/metrics?userId=${user.id}`),
          fetch(`/api/v2/dashboard/recent-activity?userId=${user.id}`),
  ...
  useEffect(() => {
    const fetchMarketInsights = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/v2/dashboard/market-insights?userId=${user.id}&marketKey=${selectedMarket}`);
        if (!response.ok) {
