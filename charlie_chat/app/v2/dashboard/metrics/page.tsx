  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const todayResponse = await fetch(`/api/v2/metrics/offers-daily?date=${today}`);
        if (todayResponse.ok) {
        ...
        const weeklyResponse = await fetch(`/api/v2/metrics/analysis-over-time?days=${timeRange}`);
