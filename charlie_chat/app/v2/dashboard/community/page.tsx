...
export default function CommunityPage() {
  ...
  useEffect(() => {
    const fetchCommunityData = async () => {
      setIsLoading(true);
      try {
        const response = await fetch('/api/v2/metrics/community-insights');
        if (response.ok) {
...
function ActivityTrendsChart({ timeRange }: { timeRange: string }) {
  ...
  useEffect(() => {
    const fetchTrendsData = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`/api/v2/metrics/community-activity-trends?timeRange=${timeRange}`);
        if (response.ok) {
          const data = await response.json();
          setChartData(data || []);
        } else {
...
function RegionalHeatMap({ timeRange }: { timeRange: string }) {
  ...
  useEffect(() => {
    const fetchHeatmapData = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`/api/v2/metrics/property-heatmap?timeRange=${timeRange}`);
        if (response.ok) {
          const data = await response.json();
          setHeatmapData(data || []);
        } else {
...
function MarketInsights() {
  ...
  const fetchInsights = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/v2/market-insights');
      if (response.ok) {
        const data = await response.json();
        setInsights(data);
      } else {
