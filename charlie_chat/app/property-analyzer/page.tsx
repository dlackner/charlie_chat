...
const calculateOverallGrade = (metrics: GradeMetrics): string => {
  ...
  // Lower (Potentially Growth Market)
  else capRateScore = 20;
  // Very Low Cap (Potentially Speculative)
...
export default function PropertyAnalyzerPage() {
  ...
  const effectiveGrossIncome = useMemo(() => {
    return (grossPotentialRent * (1 - vacancyRate / 100)) + otherIncomeAnnual - incomeReductionsAnnual; // Modified line to subtract incomeReductionsAnnual
  }, [grossPotentialRent, vacancyRate, otherIncomeAnnual, incomeReductionsAnnual]);
  // Added otherIncomeAnnual and incomeReductionsAnnual to dependencies
  ...
  const [breakEvenYear, setBreakEvenYear] = useState<number |
  null>(null);
  ...
  // New state for Overall Grade
  const [overallGrade, setOverallGrade] = useState<string>('N/A');

  // New state for Loan Balance (at Year 1, for display)
  ...
  useEffect(() => {
    ...
    let currentOtherIncomeAnnual = otherIncomeAnnual; // Initialize for projection
    let currentIncomeReductionsAnnual = incomeReductionsAnnual; // Initialize for projection

    // Set Year 1 loan balance
    // This calculation is for the *start* of Year 1, which is effectively the loanAmount
    // If you want the balance *at the end* of Year 1, you'd calculate it within the loop for y=1
    setYear1LoanBalance(loanAmount);


    for (let y = 1;
    y <= holdingPeriodYears; y++) {
      // Project Rent Growth
      currentGrossPotentialRent *= (1 + annualRentalGrowthRate / 100);
      // Other Income and Income Reductions do not grow by annualRentalGrowthRate, they are fixed amounts each year unless specified
      // For simplicity, they remain constant. If growth is desired, another input for otherIncomeGrowthRate and incomeReductionsGrowthRate is needed.
      currentEffectiveGrossIncome = (currentGrossPotentialRent * (1 - vacancyRate / 100)) + currentOtherIncomeAnnual - currentIncomeReductionsAnnual; // Updated with otherIncomeAnnual and incomeReductionsAnnual
      ...
      // This calculates the loan balance *at the end* of the current year 'y'
    ...
    if (!ArrayOfNumbers(cashFlowsForIRR) || cashFlowsForIRR.length === 0) {
    ...
    numUnits, avgMonthlyRentPerUnit, vacancyRate, annualRentalGrowthRate, otherIncomeAnnual, incomeReductionsAnnual, // Added otherIncomeAnnual and incomeReductionsAnnual to dependencies
    ...
    cashFlowBeforeTax, annualCapitalReserveTotal,
    // Add all calculated metrics to dependencies to ensure grade updates
    capRate, cashOnCashReturn, debtServiceCoverageRatio, // roi and irr are already included
  
  ]);

  // Helper function for Array.isArray check within useEffect to avoid a new function every render
  const ArrayOfNumbers = (arr: number[]): arr is number[] => Array.isArray(arr) && arr.every(item => typeof item === 'number');
    ...
    <div className="flex flex-col lg:flex-row p-4 md:p-8 bg-white text-gray-800 min-h-screen">
      <div className="lg:w-2/3 pr-0 lg:pr-8 mb-8 lg:mb-0">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="p-3 rounded-lg shadow-xl flex items-center text-white" style={{ backgroundColor: '#1C599F' }}> {/* Removed justify-between from here as it's no longer needed to separate internal 
          elements */}
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10">
                <img src="/charlie.png" alt="Grade Icon" className="w-full h-full object-contain" />
              </div>
              {/* Combined text and grade into a single p tag to ensure correct spacing */}
            ...
            <LineChart data={chartData} margin={{ top: 10, right: 30, left: 30, bottom: 30 }}>
              ...
              <Legend
                ...
                payload={[
                  ...
                  { value: 'Annual NOI', type: 
                  'line', id: 'ID02', color: '#4A90E2' },
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-5 rounded-xl shadow-lg border border-gray-200">
            ...
            <p
              className="text-2xl font-bold text-gray-900">{formatCurrency(annualDebtService)}</p>
          </div>
          <div className="bg-white p-5 rounded-xl shadow-lg border border-gray-200">
            <h3 className="text-md font-semibold text-orange-600 mb-1">Return 
            on Investment (at Year {holdingPeriodYears})</h3>
          ...
          <div className="bg-white p-5 rounded-xl shadow-lg border border-gray-200">
            ...
            <p className="text-2xl font-bold text-gray-900">
              {breakEvenYear !== null ?
              `${breakEvenYear} years` : 'N/A (No positive cash flow within horizon)'}
            </p>
          </div>
          <div className="bg-white p-5 rounded-xl shadow-lg border border-gray-200">
      ...
      <div className="lg:w-1/3 bg-gray-50 p-6 rounded-xl shadow-2xl border border-gray-200 lg:sticky lg:top-8 self-start max-h-[calc(100vh-4rem)] overflow-y-auto">
        ...
        <div className="mb-5">
          <label htmlFor="downPaymentPercentage"
            ...
            className="block text-sm font-medium text-gray-700 mb-1">Down Payment (%)</label>
          <input
            type="number"
        ...
        <div className="mb-5">
          ...
          <input
            ...
            border border-gray-300 rounded-lg text-gray-900 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-shadow duration-150 ease-in-out shadow-sm"
            step="100"
            suppressHydrationWarning={true}
          />
        </div>

        {/* New input for Income Reductions */}
        <div className="mb-5">
        ...
        <div className="mb-5">
          <label htmlFor="propertyManagementFeePercentage"
            className="block text-sm font-medium text-gray-700 mb-1">Property Management Fee (%)</label>
        ...
        <div className="mb-5">
          <label htmlFor="contractServicesAnnual"
 
            className="block text-sm font-medium text-gray-700 mb-1">Contract Services ($)</label>
          <input
            type="text"
            ...
            className="w-full p-2.5 bg-white border border-gray-300 rounded-lg text-gray-900 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-shadow 
        ...
        <div className="mb-5">
          ...
          <label htmlFor="marketingAnnual" 
          className="block text-sm font-medium text-gray-700 mb-1">Marketing ($)</label>
          <input
            type="text"
            id="marketingAnnual"
            value={(marketingAnnual ?? 0).toLocaleString('en-US')}
            onChange={formatAndParseNumberInput(setMarketingAnnual)}
            className="w-full p-2.5 bg-white border border-gray-300 rounded-lg text-gray-900 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-shadow duration-150 ease-in-out
        ...
        <div className="mb-5">
          <label htmlFor="otherExpensesAnnual"
 
            className="block text-sm font-medium text-gray-700 mb-1">Other Expenses ($)</label>
          <input
            type="text"
        ...
        <div className="mb-5">
          ...
          <input
            ...
            p-2.5 bg-white border border-gray-300 rounded-lg text-gray-900 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-shadow duration-150 ease-in-out shadow-sm"
            step="10"
            suppressHydrationWarning={true}
          />
        </div>
