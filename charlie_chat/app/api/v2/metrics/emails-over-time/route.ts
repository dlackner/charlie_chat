...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get email activity data for the specified time range
    const { data: activityData, error } = await supabase
      .from('user_activity_counts')
      .select('activity_date, count')
      .eq('user_id', userId)
      .eq('activity_type', 'emails_sent')
      .gte('activity_date', startDate.toISOString().split('T')[0])
      .order('activity_date', { ascending: true });

    if (error) {
      console.error('Error fetching email activity data:', error);
    ...
    console.error('Error in emails over time API:', error);
