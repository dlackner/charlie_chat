/*
 * CHARLIE2 V2 - Weekly Recommendations API
 * Automated property recommendation generation using MMR algorithm
 * Supports both cron-based automation and manual user triggers
 */
...
export async function POST(req: NextRequest) {
  try {
    console.log('🚀 Weekly recommendations V2 API called');
...
export async function GET() {
  return NextResponse.json({
    message: 'Weekly recommendations V2 API is active',
    endpoint: 'POST /api/v2/weekly-recommendations',
    requiresAuth: true,
    version: '2.0'
  });
}